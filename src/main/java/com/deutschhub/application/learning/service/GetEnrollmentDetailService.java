...
import com.deutschhub.domain.learning.enrollment.model.valueobject.Progress;
...
public class GetEnrollmentDetailService implements GetEnrollmentDetailUseCase {

    EnrollmentRepositoryPort enrollmentRepositoryPort;
    LessonCompletionRepositoryPort lessonCompletionRepositoryPort;

    @Override
    public AdminEnrollmentDetailResponse getEnrollmentDetail(UUID enrollmentId) {
        ...
        List<CompletedLessonDetailResponse> completedLessonDetails =
                lessonCompletionRepositoryPort.findByEnrollmentId(enrollment.getId())
                        .stream()
                        .sorted(Comparator.comparing(LessonCompletion::getCompletedAt).reversed())
                        .map(this::toCompletedLessonDetailResponse)
                        .toList();

        return toResponse(enrollment, completedLessonDetails);
    }
