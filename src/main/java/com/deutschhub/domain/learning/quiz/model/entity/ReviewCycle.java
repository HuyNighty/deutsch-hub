...
import com.deutschhub.domain.learning.quiz.model.enums.ReviewResult;
...
public class ReviewCycle {
    ...
    public static ReviewCycle restore(UUID id, UserId submittedBy, Instant submittedAt, UserId reviewedBy, Instant reviewedAt,
                                      UserId withdrawnBy, Instant withdrawnAt, ReviewResult result, ReviewFeedback feedback) {
    ...
    public void markWithdrawn(UserId withdrawnBy, Instant withdrawnAt) {
        ...
        if (withdrawnBy == null || withdrawnAt == null) {
            throw new BusinessException(ErrorCode.INVALID_QUIZ_REVIEW_WITHDRAWAL_DATA);
        }
    ...
    private void completeReview(UserId reviewer, Instant reviewedAt, ReviewResult result, ReviewFeedback feedback) {
        ...
        if (reviewer == null || reviewedAt == null) {
            throw new BusinessException(ErrorCode.INVALID_QUIZ_REVIEW_COMPLETION_DATA);
        }

        if (result == null) {
            throw new BusinessException(ErrorCode.INVALID_QUIZ_REVIEW_RESULT);
        }
    ...
    private static void validateRestoredState(UUID id, UserId submittedBy, Instant submittedAt, UserId reviewedBy,
                                              Instant reviewedAt, UserId withdrawnBy, Instant withdrawnAt, ReviewResult result, ReviewFeedback feedback) {
        if (id == null || submittedBy == null || submittedAt == null || result == null) {
            throw new BusinessException(ErrorCode.INVALID_QUIZ_REVIEW_CYCLE_DATA);
        }

        switch (result) {
            case PENDING -> requirePendingState(reviewedBy, reviewedAt, withdrawnBy, withdrawnAt,feedback);
            case APPROVED -> requireApprovedState(reviewedBy, reviewedAt, withdrawnBy, withdrawnAt, feedback);
            case CHANGES_REQUESTED -> requireChangesRequestedState(reviewedBy, reviewedAt, withdrawnBy, withdrawnAt, feedback);
            case WITHDRAWN -> requireWithdrawnState(reviewedBy, reviewedAt, withdrawnBy, withdrawnAt, feedback);
        }
    }
